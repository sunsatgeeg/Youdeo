...
// 구독 유저 영상 > 스크롤 맨 아래일때 영상 요청
$(window).scroll(function() {
	if($(window).scrollTop() + $(window).height() == $(document).height()) {
		$('.container-fluid .subvideo-section>.row>.col-md-12').each(function(){
			var start_no = parseInt($('#video_last_no').val())+1;
			var end_no = start_no+3;
			
			$.ajax({
				type : 'GET',
				url : 'subscription_video_load?start_no=' + start_no + '&end_no=' + end_no,
				dataType : 'json',
				success : function(jsonObject){
					...
					$('#video_last_no').val(jsonObject.lastNo);
					changeDate();
					chageView();
					changePlayTime();
				}
...
// Index > 스크롤 맨 아래일때 영상 요청
$(window).scroll(function() {
	if($(window).scrollTop() + $(window).height() == $(document).height()) {
		$('.container-fluid .video-section>.row>.col-md-12').each(function(){
			var start_no = parseInt($('#video_last_no').val());
			var end_no = start_no+3;
			
			$.ajax({
				type : 'GET',
				url : 'video_load?start_no=' + start_no + '&end_no=' + end_no,
				dataType : 'json',
				success : function(jsonObject){
					if(jsonObject.count==0)return;
					var html='';
...
// Watch > 스크롤 맨 아래일때 영상 요청
$(window).scroll(function() {
	if($(window).scrollTop() + $(window).height() == $(document).height()) {
		$('.single-video-right').each(function(){
			var start_no = parseInt($('#video_last_no').val());
			var end_no = start_no+1;
